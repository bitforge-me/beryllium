...
app.config['LOGO_EMAIL_SRC'] = strdef(
    'LOGO_EMAIL_SRC',
    f"http://{os.getenv('SERVER_NAME', 'error')}/static/assets/img/logo.png",
)

app.config['APPLE_APP_STORE_URL'] = strdef(
    'APPLE_APP_STORE_URL', 'https://apps.apple.com/nz/app/zap/XXX'
)
app.config['GOOGLE_PLAY_STORE_URL'] = strdef(
    'GOOGLE_PLAY_STORE_URL', 'https://play.google.com/store/apps/details?id=XXX'
)

app.config['USE_REFERRALS'] = boolify(os.getenv('USE_REFERRALS'))
app.config['REFERRAL_REWARD_TYPE_SENDER'] = strdef(
    'REFERRAL_REWARD_TYPE_SENDER', 'fixed'
)
app.config['REFERRAL_REWARD_SENDER'] = int(os.getenv('REFERRAL_REWARD_SENDER', 1000))
app.config['REFERRAL_REWARD_TYPE_RECIPIENT'] = strdef(
    'REFERRAL_REWARD_TYPE_RECIPIENT', 'fixed'
)
app.config['REFERRAL_REWARD_RECIPIENT'] = int(
    os.getenv('REFERRAL_REWARD_RECIPIENT', 1000)
)
app.config['REFERRAL_RECIPIENT_MIN_SPEND'] = int(
    os.getenv('REFERRAL_RECIPIENT_MIN_SPEND', 5000)
)
...
app.config['MIN_AVAILABLE_NZD_BALANCE'] = decimal.Decimal(
    os.getenv('MIN_AVAILABLE_NZD_BALANCE', 2000)
)
...
def set_vital_setting(
    env_name, setting_name=None, acceptable_values=None, custom_handler=None
):
    ...
    if os.getenv(env_name):
        ...
        if acceptable_values and value not in acceptable_values:
            print(
                env_name
                + ' not in range of acceptable values: '
                + str(acceptable_values)
            )
...
set_vital_setting(
    'PASSWORD_SALT', 'SECURITY_PASSWORD_SALT', custom_handler=set_totp_secret
)
...
limiter = Limiter(
    ...
    default_limits=['3000 per minute'],
)
