...
def user_info_dict(api_key: ApiKey, all_info: bool) -> dict:
    ...
    return dict(
        ...
        kyc_url=kyc_url,
        aplyid_req_exists=aplyid_req_exists,
...
def broker_order_new_event(broker_order: BrokerOrder):
    data = json.dumps(broker_order.to_json())
    socketio.emit(
        'broker_order_new', data, json=True, room=broker_order.user.email, namespace=NS
    )
...
def crypto_deposit_update_event(crypto_deposit: CryptoDeposit):
    ...
    socketio.emit(
        ...
        data,
        json=True,
        room=crypto_deposit.user.email,
        namespace=NS,
    )
...
def crypto_withdrawal_update_event(crypto_withdrawal: CryptoWithdrawal):
    ...
    socketio.emit(
        'crypto_withdrawal_update',
        data,
        json=True,
        room=crypto_withdrawal.user.email,
        namespace=NS,
    )
...
def fiat_deposit_new_event(fiat_deposit: FiatDeposit):
    data = json.dumps(fiat_deposit.to_json())
    socketio.emit(
        'fiat_deposit_new', data, json=True, room=fiat_deposit.user.email, namespace=NS
    )
...
def fiat_withdrawal_update_event(fiat_withdrawal: FiatWithdrawal):
    ...
    socketio.emit(
        'fiat_withdrawal_update',
        data,
        json=True,
        room=fiat_withdrawal.user.email,
        namespace=NS,
    )
...
def ln_invoice_paid_event(
    ...
    logger.info(
        'ln_invoice_paid: %s, %s, %s, %s', label, email, description, amount_sat
    )
...
class EventsNamespace(Namespace):
    ...
    def on_connect(self):
        logger.info(
            'connect sid: %s', request.sid  # pyright: ignore [reportGeneralTypeIssues]
        )
        emit(
            'version',
            json.dumps(
    ...
    def on_auth(self, auth: dict | str):
        ...
        if res and api_key:
            ...
            emit(
                'info',
                f'failed authentication ({api_key_token}): {reason}',
                namespace=NS,
            )
