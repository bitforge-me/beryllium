...
BTCLN = Asset(
    ...
    decimals=8,
    withdraw_fee=Dec('0.005'),
    withdraw_fee_fixed=False,
    min_withdraw=Dec('0.00000001'),
    is_crypto=True,
    l2_network=None,
    deposit_instr=None,
    withdraw_instr=None,
...
USDT = Asset(
    ...
    is_crypto=True,
    l2_network=None,
    deposit_instr='This is an ethereum network address. Only deposit from the ethereum network.',
    withdraw_instr='Only withdraw to an ethereum address on the ethereum network.',
)
USDC = Asset(
    symbol='USDC',
    name='USD Coin',
...
ETH = Asset(
    ...
    name='Ethereum',
    decimals=18,
    withdraw_fee=Dec('0.0052'),
    withdraw_fee_fixed=True,
    min_withdraw=Dec('0.01'),
    is_crypto=True,
    l2_network=None,
    deposit_instr=None,
    withdraw_instr=None,
...
LTC = Asset(
    ...
    name='Litecoin',
    decimals=8,
    withdraw_fee=Dec('0.01'),
    withdraw_fee_fixed=True,
    min_withdraw=Dec('0.03'),
    is_crypto=True,
    l2_network=None,
    deposit_instr=None,
    withdraw_instr=None,
...
WAVES = Asset(
    ...
    name='Waves',
    decimals=8,
    withdraw_fee=Dec('0.001'),
    withdraw_fee_fixed=True,
    min_withdraw=Dec('0.003'),
    is_crypto=True,
    l2_network=None,
    deposit_instr=None,
    withdraw_instr=None,
)
ASSETS_HISTORICAL = dict(
    NZD=NZD, BTC=BTC, USDT=USDT, USDC=USDC, ETH=ETH, DOGE=DOGE, LTC=LTC, WAVES=WAVES
)
ASSETS = dict(NZD=NZD, BTC=BTC)
MARKETS_HISTORICAL = {
    'BTC-NZD': Market(base_asset=BTC, quote_asset=NZD),
...
def _base58_validate(
    address: str, mainnet_prefixes: list[int], testnet_prefixes: list[int]
) -> bool:
    try:
        ...
        return (
            not TESTNET
            and prefix in mainnet_prefixes
            or TESTNET
            and prefix in testnet_prefixes
        )
    except ValueError:
        return False
...
def asset_withdraw_fee(
    asset: str, l2_network: str | None, amount: Dec | None = None
) -> Dec:
...
def asset_has_l2(asset: str, l2_network: str | None) -> bool:
    ...
    for item in ASSETS.values():
        if (
            item.symbol == asset
            and item.l2_network is not None
            and item.l2_network.symbol == l2_network
        ):
            return True
    return False
...
def asset_recipient_extract_amount(
    asset: str, l2_network: str | None, recipient: str
) -> Dec:
