...
def request_start(token=None):
    ...
    if request.method == 'POST':
        ...
        if aplyid_phone:
            logger.info(
                'kyc_request token: %s, aplyid_phone: %s', req.token, aplyid_phone
            )
    ...
    return render_template(
        ...
        aplyid_transaction_id=aplyid_transaction_id,
...
def aplyid_webhook():
    ...
    if (
        len(parts) != 2
        or parts[0] != 'Bearer'
        or parts[1] != APLYID_WEBHOOK_BEARER_TOKEN
    ):
        return abort(403)
    ...
    if (
        (data['event'] == 'completed' or data['event'] == 'updated')
        and 'verification' in data
        and (
            data['verification']['status'] == 'pass'
            or data['verification']['status'] == 'reviewed'
        )
    ):
