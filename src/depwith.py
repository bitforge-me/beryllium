...
def withdrawal_cancel(withdrawal: CryptoWithdrawal | FiatWithdrawal, reason: str):
    ...
    return fiatdb_core.tx_create(
        withdrawal.user,
        FiatDbTransaction.ACTION_CREDIT,
        asset,
        amount_int,
        f'{type_} withdrawal refund - {reason}: {withdrawal.token}',
    )
...
def _fiat_deposit_update(fiat_deposit: FiatDeposit):
    logger.info(
        'processing fiat deposit %s (%s)..', fiat_deposit.token, fiat_deposit.status
    )
    updated_records = []
    # check payment
    if fiat_deposit.status == fiat_deposit.STATUS_CREATED:
        if fiat_deposit.windcave_payment_request:
            ...
            if payment_req.status == payment_req.STATUS_COMPLETED:
                ...
                ftx = fiatdb_core.tx_create(
                    fiat_deposit.user,
                    FiatDbTransaction.ACTION_CREDIT,
                    fiat_deposit.asset,
                    fiat_deposit.amount,
                    f'fiat deposit: {fiat_deposit.token}',
                )
                updated_records.append(payment_req)
                updated_records.append(fiat_deposit)
    ...
    if (
        fiat_deposit.status == fiat_deposit.STATUS_CREATED
        and fiat_deposit.windcave_payment_request
    ):
...
def _fiat_deposit_email(fiat_deposit: FiatDeposit):
    if fiat_deposit.status == fiat_deposit.STATUS_COMPLETED:
        email_utils.send_email(
            logger,
            'Deposit Completed',
            _fiat_deposit_email_msg(fiat_deposit, ''),
            fiat_deposit.user.email,
        )
...
def _crown_check_new_desposits(db_session: Session):
    if not utils.is_email(crown_financial.EMAIL):
        ...
        return
    txs = crown_financial.transactions_filtered_type(
        crown_financial.CrownTx.TYPE_DEPOSIT,
        datetime.now() - timedelta(days=7),
        datetime.now(),
    )
    for tx in txs:
        if (
            tx.currency == crown_financial.CURRENCY
            and not CrownPayment.from_crown_txn_id(db_session, tx.crown_txn_id)
        ):
            payment = CrownPayment(
                utils.generate_key(), tx.currency, tx.amount, tx.crown_txn_id, tx.status
            )
...
def _fiat_deposit_update_and_commit(db_session: Session, deposit: FiatDeposit):
    if deposit.asset not in assets.ASSETS:
        logger.error(
            'fiat deposit (%s) asset (%s) is not valid', deposit.token, deposit.asset
        )
        return
    while True:
...
def _fiat_withdrawal_update(fiat_withdrawal: FiatWithdrawal):
    logger.info(
        'processing fiat withdrawal %s (%s)..',
        fiat_withdrawal.token,
        fiat_withdrawal.status,
    )
    ...
    if fiat_withdrawal.status == fiat_withdrawal.STATUS_CREATED:
        ...
        if not conf:
            logger.error(
                'fiat withdrawal (%s) does not have a confirmation record',
                fiat_withdrawal.token,
            )
            return updated_records
        elif conf.confirmed is None:
            if conf.expired():
                ftx = withdrawal_cancel(
                    fiat_withdrawal, 'confirmation expiry'
    ...
    if fiat_withdrawal.status == fiat_withdrawal.STATUS_AUTHORIZED:
        if not fiat_withdrawal.payout_request:
            ...
            assert address_book
            payout_request = payouts_core.payout_create(
                fiat_withdrawal.amount,
                crown_financial.CROWN_WITHDRAW_NAME,
                fiat_withdrawal.token,
                address_book,
            )
...
def _fiat_withdrawal_email(fiat_withdrawal: FiatWithdrawal):
    if fiat_withdrawal.status == fiat_withdrawal.STATUS_COMPLETED:
        email_utils.send_email(
            ...
            _fiat_withdrawal_email_msg(fiat_withdrawal, ''),
            fiat_withdrawal.user.email,
        )
...
def _fiat_withdrawal_update_and_commit(db_session: Session, withdrawal: FiatWithdrawal):
    if withdrawal.asset not in assets.ASSETS:
        logger.error(
            'fiat withdrawal (%s) asset (%s) is not valid',
            withdrawal.token,
            withdrawal.asset,
        )
        return
...
def _crypto_deposit_email(deposit: CryptoDeposit):
    if deposit.confirmed:
        email_utils.send_email(
            logger,
            'Deposit Confirmed',
            _crypto_deposit_email_msg(deposit, 'confirmed', ''),
            deposit.user.email,
        )
    else:
        email_utils.send_email(
            logger,
            'Deposit Incoming',
            _crypto_deposit_email_msg(deposit, 'incoming', ''),
            deposit.user.email,
        )


def _crypto_deposits_wallet_check(
    db_session: Session,
        ...
        for wallet_deposit in wallet_deposits:
            ...
            if not crypto_deposit:
                crypto_deposit = CryptoDeposit(
                    ...
                    asset,
                    None,
                    wallet_deposit.amount_deposited(),
                    None,
                    wallet_deposit.txid,
                    wallet_deposit.txid,
                    completed,
            ...
            if (
                crypto_deposit in new_crypto_deposits or not crypto_deposit.confirmed
            ) and completed:
                ...
                ftx = fiatdb_core.tx_create(
                    ...
                    asset,
                    wallet_deposit.amount_deposited(),
                    f'crypto deposit: {crypto_deposit.token}',
                )
                db_session.add(ftx)
...
def _crypto_deposits_dasset_check(
    db_session: Session,
    new_crypto_deposits: list[CryptoDeposit],
    updated_crypto_deposits: list[CryptoDeposit],
    user: User,
    ...
    if not user.dasset_subaccount:
        ...
        return
    dasset_deposits = dasset.crypto_deposits(
        asset, user.dasset_subaccount.subaccount_id
    )
    for dasset_deposit in dasset_deposits:
        ...
        if (
            ...
            if not dasset.transfer(
                None, user.dasset_subaccount.subaccount_id, asset, dasset_deposit.amount
            ):
                logger.error(
                    'failed to transfer funds from subaccount to master %s',
                    dasset_deposit.id,
                )
                continue
...
def _crypto_deposits_address_check(
    db_session: Session,
    new_crypto_deposits: list[CryptoDeposit],
    updated_crypto_deposits: list[CryptoDeposit],
):
...
def _crypto_deposits_updated_wallet_check(
    db_session: Session, updated_crypto_deposits: list[CryptoDeposit]
):
    for deposit in CryptoDeposit.of_wallet(db_session, False, False):
        logger.info(
            'processing crypto deposit %s (confirmed: %s)..',
            deposit.token,
            deposit.confirmed,
        )
        if wallet.deposit_expired(
            deposit.asset, deposit.l2_network, deposit.wallet_reference
        ):
            deposit.expired = True  # has no status field :(((
            updated_crypto_deposits.append(deposit)
        elif wallet.deposit_completed(
            deposit.asset, deposit.l2_network, deposit.wallet_reference
        ):
            ...
            ftx = fiatdb_core.tx_create(
                ...
                deposit.asset,
                deposit.amount,
                f'crypto deposit: {deposit.token}',
            )
            db_session.add(ftx)
            updated_crypto_deposits.append(deposit)
        db_session.add(deposit)
...
def crypto_deposits_check(db_session: Session):
    ...
    with coordinator.lock:
        ...
        _crypto_deposits_address_check(
            db_session, new_crypto_deposits, updated_crypto_deposits
        )
        # check crypto deposits in our wallet
        _crypto_deposits_updated_wallet_check(db_session, updated_crypto_deposits)
    # send updates
...
def _crypto_withdrawal_update(crypto_withdrawal: CryptoWithdrawal):
    logger.info(
        'processing crypto withdrawal %s (%s)..',
        crypto_withdrawal.token,
    ...
    if crypto_withdrawal.status == crypto_withdrawal.STATUS_CREATED:
        ...
        if not conf:
            logger.error(
                'crypto withdrawal (%s) does not have a confirmation record',
                crypto_withdrawal.token,
            )
        elif conf.confirmed is None:
            if conf.expired():
                ftx = withdrawal_cancel(
                    crypto_withdrawal, 'confirmation expiry'
                )  # sets status to STATUS_CANCELLED
    ...
    if crypto_withdrawal.status == crypto_withdrawal.STATUS_AUTHORIZED:
        if (
            not crypto_withdrawal.wallet_reference
            and not crypto_withdrawal.exchange_reference
        ):
            ...
            if wallet.withdrawals_supported(asset, l2_network):
                ...
                wallet_reference, err_msg = wallet.withdrawal_create(
                    asset, l2_network, amount_dec, recipient
                )
                if err_msg:
                ...
                if not dasset.funds_available_us(asset, amount_dec):
                    ...
                    return updated_records
                exchange_reference = dasset.crypto_withdrawal_create(
                    asset, amount_dec, recipient
                )
    ...
    if crypto_withdrawal.status == crypto_withdrawal.STATUS_WITHDRAW:
        if crypto_withdrawal.wallet_reference:
            ...
            if wallet.withdrawal_completed(
                crypto_withdrawal.asset,
                crypto_withdrawal.l2_network,
                crypto_withdrawal.wallet_reference,
            ):
            ...
            status = dasset.crypto_withdrawal_status_check(
                crypto_withdrawal.exchange_reference
            )
            if status == dasset.CRYPTO_WITHDRAWAL_STATUS_COMPLETED:
                crypto_withdrawal.status = crypto_withdrawal.STATUS_COMPLETED
                updated_records.append(crypto_withdrawal)
            elif status == dasset.CRYPTO_WITHDRAWAL_STATUS_2FA:
                if not dasset.crypto_withdrawal_confirm(
                    crypto_withdrawal.exchange_reference
                ):
                    logger.error(
                        'failed to confirm crypto withdrawal %s',
                        crypto_withdrawal.token,
                    )
            elif status == dasset.CRYPTO_WITHDRAWAL_STATUS_UNKNOWN:
                logger.error(
                    'failed to get crypto withdrawal %s status', crypto_withdrawal.token
                )
        return updated_records
    return updated_records
...
def _crypto_withdrawal_email(crypto_withdrawal: CryptoWithdrawal):
    if crypto_withdrawal.status == crypto_withdrawal.STATUS_COMPLETED:
        email_utils.send_email(
            logger,
            'Withdrawal Completed',
            _crypto_withdrawal_email_msg(crypto_withdrawal, ''),
            crypto_withdrawal.user.email,
        )


def _crypto_withdrawal_update_and_commit(
    db_session: Session, withdrawal: CryptoWithdrawal
):
    if withdrawal.asset not in assets.ASSETS:
        logger.error(
            'crypto withdrawal (%s) asset (%s) is not valid',
            withdrawal.token,
            withdrawal.asset,
        )
        return
