...
def order_refund(order: BrokerOrder):
    ...
    return fiatdb_core.tx_create(
        ...
        asset,
        amount_int,
        f'broker order refund: {order.token}',
    )
...
def order_check_funds(db_session: Session, order: BrokerOrder, check_user: bool = True):
    ...
    if check_user and not fiatdb_core.funds_available_user(
        db_session, order.user, asset, amount_dec
    ):
...
def _broker_order_action(db_session: Session, broker_order: BrokerOrder):
    logger.info(
        'processing broker order %s (%s)..', broker_order.token, broker_order.status
    )
    updated_records = []
    base_amount_dec = assets.asset_int_to_dec(
        broker_order.base_asset, broker_order.base_amount
    )
    quote_amount_dec = assets.asset_int_to_dec(
        broker_order.quote_asset, broker_order.quote_amount
    )
    ...
    if not side:
        logger.error(
            'invalid side "%s" for broker order %s',
            broker_order.side,
            broker_order.token,
        )
    ...
    if broker_order.status == broker_order.STATUS_READY:
        ...
        if err_msg:
            ...
            if not ftx:
                logger.error(
                    'failed to create fiatdb transaction for broker order %s',
                    broker_order.token,
                )
                return updated_records
        ...
        exchange_order_id = dasset.order_create(
            broker_order.market, side, base_amount_dec, price
        )
    ...
    if broker_order.status == broker_order.STATUS_EXCHANGE:
        ...
        if broker_order.exchange_order:
            if dasset.order_status_check(
                broker_order.exchange_order.exchange_reference, broker_order.market
            ):
                ...
                ftx = fiatdb_core.tx_create(
                    ...
                    asset,
                    amount_int,
                    f'broker order completed: {broker_order.token}',
                )
                updated_records.append(ftx)
...
def _email_msg(broker_order: BrokerOrder, msg: str):
    ...
    amount_dec = assets.asset_int_to_dec(
        broker_order.base_asset, broker_order.base_amount
    )
...
def _broker_order_email(broker_order: BrokerOrder):
    if broker_order.status == broker_order.STATUS_FAILED:
        email_utils.send_email(
            logger,
            'Order Failed',
            _email_msg(broker_order, ''),
            broker_order.user.email,
        )
...
def _broker_order_update_and_commit(db_session: Session, broker_order: BrokerOrder):
    if broker_order.market not in assets.MARKETS:
        logger.error(
            'broker order (%s) market (%s) is not valid',
            broker_order.token,
            broker_order.market,
        )
        return
