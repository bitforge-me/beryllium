...
def _parse_balance(item):
    ...
    return DassetBalance(
        symbol=symbol,
        name=item['currencyName'],
        total=decimal.Decimal(item['total']),
        available=decimal.Decimal(item['available']),
        decimals=assets.asset_decimals(symbol),
    )
...
def _parse_asset(item):
    ...
    return DassetAsset(
        ...
        status=item['status'],
        min_confs=item['minConfirmations'],
        message=message,
        decimals=assets.asset_decimals(symbol),
    )
...
def _parse_market(item):
    ...
    return DassetMarket(
        symbol=item['symbol'],
        base_asset=item['baseCurrencySymbol'],
        quote_asset=item['quoteCurrencySymbol'],
        precision=item['precision'],
        status=item['status'],
        min_trade=item['minTradeSize'],
        message=message,
    )
...
def _parse_order(item):
    ...
    return DassetOrder(
        id=item['id'],
        base_asset=item['baseSymbol'],
        quote_asset=item['quoteSymbol'],
        date=item['timestamp'],
        side=side,
        status=item['status'],
        base_amount=item['baseAmount'],
        quote_amount=item['quoteAmount'],
        filled=item['details']['filled'],
    )


def _parse_deposit(item):
    return DassetDeposit(
        id=item['id'],
        symbol=item['currencySymbol'],
        address=item['cryptoAddress'],
        amount=decimal.Decimal(item['quantity']),
        date=item['updatedAt'],
        status=item['status'],
        txid=item['txId'],
    )
...
def _parse_withdrawal_2fa(item):
    ...
    return DassetWithdrawal(
        id=id_, symbol='', amount='0', date='', status=status, address=''
    )
...
def _req_post(
    endpoint: str, params, subaccount_id: str | None = None, noapi_in_path=False
):
    url = URL_BASE + endpoint
    if noapi_in_path:
        url = URL_BASE_NOAPI + endpoint
...
def _balances_req(asset: str | None, subaccount_id: str | None):
    ...
    if r.status_code == 200:
        balances = r.json()
        balances = [
            _parse_balance(b) for b in balances if b['currencySymbol'] in assets.ASSETS
        ]
        return balances
    logger.error('request failed: %d, %s', r.status_code, r.content)
    return None
...
def _crypto_withdrawal_create_req(asset: str, amount: decimal.Decimal, address: str):
    ...
    r = _req_post(
        endpoint,
        params=dict(
            currencySymbol=asset, quantity=float(amount), cryptoAddress=address
        ),
    )
    if r.status_code == 200:
...
def _crypto_withdrawal_confirm_req(withdrawal_id: str, totp_code: str):
    ...
    r = _req_post(
        endpoint, params=dict(txId=withdrawal_id, token=totp_code), noapi_in_path=True
    )
    if r.status_code == 200:
        return True
    logger.error('request failed: %d, %s', r.status_code, r.content)
...
def _addresses_create_req(asset: str, subaccount_id: str):
    endpoint = '/addresses'
    r = _req_post(
        endpoint, params=dict(currencySymbol=asset), subaccount_id=subaccount_id
    )
    if r.status_code == 200:
...
def _crypto_deposits_pending_req(asset: str, subaccount_id: str):
    ...
    r = _req_get(
        endpoint,
        params=dict(currencySymbol=asset, status='PENDING'),
...
def _crypto_deposits_closed_req(asset: str, subaccount_id: str):
    ...
    r = _req_get(
        endpoint,
        params=dict(currencySymbol=asset),
        subaccount_id=subaccount_id,
        noapi_in_path=True,
    )
    if r.status_code == 200:
...
def _transfer_req(
    to_master: bool,
    from_subaccount_id: str | None,
    to_subaccount_id: str | None,
    asset: str,
    amount: decimal.Decimal,
):
    endpoint = '/transfer'
...
def bid_quote_amount(market: str, amount: decimal.Decimal, use_cache=False):
    ...
    while amount > filled:
        ...
        if filled == amount:
            return (
                total_price * (decimal.Decimal(1) + broker_fee / decimal.Decimal(100)),
                QuoteResult.OK,
            )
        n += 1
    ...
    if _account_mock():
        ...
        for item in assets.ASSETS.values():
            if subaccount_id and assets.asset_is_fiat(item.symbol):
                continue
            balance = DassetBalance(
                symbol=item.symbol,
                name=item.name,
                total=decimal.Decimal('9999.01234567890123456789'),
                available=decimal.Decimal('9999.01234567890123456789'),
