...
from flask_security import (
    ...
    current_user,  # pyright: ignore [reportGeneralTypeIssues]
)
...
def tf_code_send(user: User) -> bool:
    if tf_method() == 'email':
        msg = user.tf_send_security_token(
            method='email', totp_secret=user.tf_totp_secret, phone_number=None
        )
        if msg:
            logger.error(
                'failed to send two factor code to user %s (%s)', user.email, msg
            )
        return not msg
    return True
...
def tf_code_validate(user: User, code: str) -> bool:
    ...
    if (
        not window
        or not user.tf_totp_secret
        or not _security._totp_factory.verify_totp(
            token=code, totp_secret=user.tf_totp_secret, user=user, window=window,
        )
    ):
        return False
...
def check_verify():
    ...
    if (
        ...
        and request.path == url_for('security.verify')
...
class SecureTwoFactorSetupForm(flask_security.forms.TwoFactorSetupForm):
    def __init__(self, *args, **kwargs):
        ...
        if (
            hasattr(current_user, 'tf_primary_method')
            and current_user.tf_primary_method
        ):
...
security = Security(
    app,
    user_datastore,
    verify_form=SecureVerifyForm,
    two_factor_setup_form=SecureTwoFactorSetupForm,
    two_factor_rescue_form=SecureTwoFactorRescueForm,
)
