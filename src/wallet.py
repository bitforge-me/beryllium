...
class BtcTxBasic:
    def __init__(
        ...
        txid: str,
        blockheight: int,
        inputs: list[BtcInputBasic],
        outputs: list[BtcOutputBasic],
        fee: int,
        has_output_ours: bool,
        has_input_ours: bool,
    ):
        self.txid = txid
...
def bitcoind_service_url():
    return _build_bitcoin_rpc_url(
        app.config['BITCOIN_DATADIR'], app.config['BITCOIN_RPCCONNECT']
    )
...
def incoming_available(asset: str, l2_network: str | None, amount_dec: Decimal) -> bool:
    if _is_ln(asset, l2_network):
        ...
        logger.info(
            'required: %d sats, largest channel theirs: %d sats',
            sats,
            funds['sats_largest_channel_theirs'],
        )
...
def funds_available(asset: str, l2_network: str | None, amount_dec: Decimal) -> bool:
    if _is_ln(asset, l2_network):
        funds = LnRpc().list_funds()
        sats = assets.asset_dec_to_int(
            asset, amount_dec * Decimal('1.01')
        )  # add a 1% buffer for fees
        logger.info(
            'required: %d sats, largest channel: %d sats',
            sats,
            funds['sats_largest_channel'],
        )
    ...
    if _is_btc_chain(asset, l2_network):
        ...
        sats = assets.asset_dec_to_int(asset, amount_dec)
        logger.info(
            'required: %d sats, sats onchain: %d sats', sats, funds['sats_onchain']
        )
...
def withdrawal_create(
    asset: str, l2_network: str | None, amount_dec: Decimal, recipient: str
):
    ...
    if _is_ln(asset, l2_network):
        try:
            ...
            if amount_sat != result['amount_sat']:
                logger.error(
                    'ln pay amount does not match: %d, %d',
                    amount_sat,
                    result['amount_sat'],
                )
...
def withdrawal_completed(
    asset: str, l2_network: str | None, wallet_reference: str
) -> bool:
    rpc = LnRpc()
    if _is_ln(asset, l2_network):
...
def deposit_create(
    asset: str, l2_network: str | None, label: str, msg: str, amount_dec: Decimal
):
...
def address_deposits(
    asset: str, l2_network: str | None, address: str
) -> list[BtcTxBasic]:
    assert deposits_supported(asset, l2_network)
    deposit_txs = []
    if _is_btc_chain(asset, l2_network):
...
def _get_raw_tx(service_url, txid, blockhash=None):
    try:
        if blockhash:
            return bitcoind_rpc_url(
                service_url, 'getrawtransaction', txid, True, blockhash
            )
...
def _get_raw_tx_using_bitaps_blockhash(service_url, txid):
    ...
    try:
        tx = requests.get(
            f'https://api.bitaps.com/btc/{network}/blockchain/transaction/{txid}'
        ).json()
        ...
        for tx in txs:
            for input_ in tx['inputs']:
                ...
                if not input_tx:
                    ...
                    input_tx = _get_raw_tx_using_loop(
                        service_url, input_txid, max_block_history
                    )
                if not input_tx:
                    logger.error(
                        'failed to find input tx after searching top %d blocks, trying bitaps.com',
                        max_block_history,
                    )
                    input_tx = _get_raw_tx_using_bitaps_blockhash(
                        service_url, input_txid
                    )
                if not input_tx:
                ...
                # make db input tx
                input_tx = BtcTxIndex(
                    input_txid, input_tx['hex'], None, input_tx['blockhash']
                )
            ...
            if addr:
                ...
                for address in addresses:
                    if output_address in (address['bech32'], address['p2sh']):
                        ...
                        has_output_ours = True
            outputs.append(
                BtcOutputBasic(output_address, output['index'], output_sats, ours)
            )
        for input_ in tx['inputs']:
            ...
            inputs.append(
                BtcInputBasic(
                    input_['txid'], input_address, input_['index'], input_sats, ours
                )
            )
