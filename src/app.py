...
def create_role(name, desc):
    ...
    if not role:
        role = Role(
            name=name, description=desc  # pyright: ignore [reportGeneralTypeIssues]
        )
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == 'seed_db':
            ...
            create_role(
                Role.ROLE_FINANCE, 'Can view all records, can authorize rewards'
            )
        ...
        web_greenlet = web.WebGreenlet(  # pyright: ignore [reportGeneralTypeIssues]
            g_exception
        )
        web_greenlet.start()
        while KEEP_RUNNING:
