...
def send_email_sendgrid(
    ...
    message = Mail(
        from_email=from_email, to_emails=recipient, subject=subject, html_content=html
    )
    if attachment:
...
def send_email_postfix(
    ...
    html = render_template('email.html', content=msg)
    message = Message(
        sender=from_email, recipients=[recipient], subject=subject, html=html
    )
    if attachment:
...
def email_api_key_request(logger: Logger, req: ApiKeyRequest):
    url = url_for(
        'api.api_key_confirm', token=req.token, secret=req.secret, _external=True
    )
...
def email_payouts_notification(logger: Logger, payout_requests: list[PayoutRequest]):
    ...
    html_content = (
        f'{len(payout_requests)} payout requests<br/><br/><a href="{url}">payouts</a>'
    )
...
def email_tripwire_notification(logger: Logger):
    ...
    html_content = (
        f'the tripwire at <a href="{server_name}">{server_name}</a> has triggered'
    )
...
def email_withdrawal_confirmation(logger: Logger, conf: WithdrawalConfirmation):
    url = url_for(
        'api.withdrawal_confirm', token=conf.token, secret=conf.secret, _external=True
    )
    asset = conf.asset()
