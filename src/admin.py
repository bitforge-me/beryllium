...
def get_broker_order_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'assets'):
            ...
            g.assets = [
                (broker_order.base_asset, broker_order.base_asset)
                for broker_order in query
            ]
...
def get_brokerorder_statuses():
    ...
    if has_app_context():
        if not hasattr(g, 'brokerorder_statuses'):
            ...
            g.brokerorder_statuses = [
                (broker_order.status, broker_order.status) for broker_order in query
            ]
...
def get_brokerorder_markets():
    ...
    if has_app_context():
        if not hasattr(g, 'brokerorder_markets'):
            ...
            g.brokerorder_markets = [
                (broker_order.market, broker_order.market) for broker_order in query
            ]
...
def get_cryptowithdrawal_statuses():
    ...
    if has_app_context():
        if not hasattr(g, 'cyptowithdrawal_statuses'):
            query = CryptoWithdrawal.query.with_entities(
                CryptoWithdrawal.status
            ).distinct()
            g.cryptowithdrawal_statuses = [
                (crypto_withdrawal.status, crypto_withdrawal.status)
                for crypto_withdrawal in query
            ]
        for (
            crypto_withdrawal_status_a,
            crypto_withdrawal_status_b,
        ) in g.cryptowithdrawal_statuses:
...
def get_cryptowithdrawal_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'cyptowithdrawal_assets'):
            query = CryptoWithdrawal.query.with_entities(
                CryptoWithdrawal.asset
            ).distinct()
            g.cryptowithdrawal_assets = [
                (crypto_withdrawal.asset, crypto_withdrawal.asset)
                for crypto_withdrawal in query
            ]
        for (
            crypto_withdrawal_asset_a,
            crypto_withdrawal_asset_b,
        ) in g.cryptowithdrawal_assets:
...
def get_cryptodeposit_confirmed():
    ...
    if has_app_context():
        if not hasattr(g, 'cyptodeposit_confirmed'):
            query = CryptoDeposit.query.with_entities(
                CryptoDeposit.confirmed
            ).distinct()
            g.cryptodeposit_confirmed = [
                (crypto_deposit.confirmed, crypto_deposit.confirmed)
                for crypto_deposit in query
            ]
        for (
            crypto_deposit_confirmed_a,
            crypto_deposit_confirmed_b,
        ) in g.cryptodeposit_confirmed:
...
def get_cryptodeposit_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'cyptodeposit_assets'):
            ...
            g.cryptodeposit_assets = [
                (crypto_deposit.asset, crypto_deposit.asset) for crypto_deposit in query
            ]
...
def get_fiatwithdrawal_statuses():
    ...
    if has_app_context():
        if not hasattr(g, 'fiatwithdrawal_statuses'):
            ...
            g.fiatwithdrawal_statuses = [
                (fiat_withdrawal.status, fiat_withdrawal.status)
                for fiat_withdrawal in query
            ]
        for (
            fiat_withdrawal_status_a,
            fiat_withdrawal_status_b,
        ) in g.fiatwithdrawal_statuses:
...
def get_fiatwithdrawal_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'fiatwithdrawal_assets'):
            ...
            g.fiatwithdrawal_assets = [
                (fiat_withdrawal.asset, fiat_withdrawal.asset)
                for fiat_withdrawal in query
            ]
...
def get_fiatdeposit_statuses():
    ...
    if has_app_context():
        if not hasattr(g, 'fiatdeposit_statuses'):
            ...
            g.fiatdeposit_statuses = [
                (fiat_deposit.status, fiat_deposit.status) for fiat_deposit in query
            ]
...
def get_fiatdeposit_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'fiatdeposit_assets'):
            ...
            g.fiatdeposit_assets = [
                (fiat_deposit.asset, fiat_deposit.asset) for fiat_deposit in query
            ]
...
def get_payoutrequest_statuses():
    ...
    if has_app_context():
        if not hasattr(g, 'payoutrequest_statuses'):
            ...
            g.payoutrequest_statuses = [
                (payout_request.status, payout_request.status)
                for payout_request in query
            ]
        for (
            payout_request_status_a,
            payout_request_status_b,
        ) in g.payoutrequest_statuses:
...
def get_payoutrequest_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'payoutrequest_assets'):
            ...
            g.payoutrequest_assets = [
                (payout_request.asset, payout_request.asset) for payout_request in query
            ]
...
def get_fiatdbtransaction_assets():
    ...
    if has_app_context():
        if not hasattr(g, 'fiatdbtransaction_assets'):
            query = FiatDbTransaction.query.with_entities(
                FiatDbTransaction.asset
            ).distinct()
            g.fiatdbtransaction_assets = [
                (fiat_db_transaction.asset, fiat_db_transaction.asset)
                for fiat_db_transaction in query
            ]
        for (
            fiat_db_transaction_asset_a,
            fiat_db_transaction_asset_b,
        ) in g.fiatdbtransaction_assets:
...
def get_fiatdbtransaction_actions():
    ...
    if has_app_context():
        if not hasattr(g, 'fiatdbtransaction_actions'):
            query = FiatDbTransaction.query.with_entities(
                FiatDbTransaction.action
            ).distinct()
            g.fiatdbtransaction_actions = [
                (fiat_db_transaction.action, fiat_db_transaction.action)
                for fiat_db_transaction in query
            ]
        for (
            fiat_db_transaction_action_a,
            fiat_db_transaction_action_b,
        ) in g.fiatdbtransaction_actions:
    ...
    def is_accessible(self):
        return (
            ...
            and (
                ...
                or current_user.has_role(Role.ROLE_FINANCE)
            )
        )
...
class UserModelView(BaseModelView):
    ...
    column_filters = [
        FilterByUserEmail(User.email, 'Search email'),
    ...
    def is_accessible(self):
        if not (current_user.is_active and current_user.is_authenticated):
            return False
        if current_user.has_role(Role.ROLE_FINANCE) and not current_user.has_role(
            Role.ROLE_ADMIN
        ):
            return True
        return False
...
class BrokerOrderModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(BrokerOrder.date, 'Search Date'),
...
class CryptoWithdrawalModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(CryptoWithdrawal.date, 'Search Date'),
...
class CryptoDepositModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(CryptoDeposit.date, 'Search Date'),
        FilterByCryptoDepositConfirmedEqual(
            CryptoDeposit.confirmed, 'Search Confirmed'
        ),
...
class FiatWithdrawalModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(FiatWithdrawal.date, 'Search Date'),
...
class FiatDepositModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(FiatDeposit.date, 'Search Date'),
...
class PayoutRequestModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(PayoutRequest.date, 'Search Date'),
...
class FiatDbTransactionModelView(RestrictedModelView):
    ...
    column_filters = [
        DateBetweenFilter(FiatDbTransaction.date, 'Search Date'),
...
admin = flask_admin.Admin(
    app, 'Beryllium Admin', base_template='my_master.html', template_mode='bootstrap3',
)

# Add model views
admin.add_view(UserModelView(User, db.session, category='Admin'))
admin.add_view(
    AdminUserModelView(User, db.session, category='Admin', endpoint='AdminUser')
)
...
admin.add_view(
    PushNotificationLocationModelView(
        PushNotificationLocation, db.session, category='Admin'
    )
)
...
admin.add_view(
    CryptoWithdrawalModelView(CryptoWithdrawal, db.session, category='Admin')
)
...
admin.add_view(
    RestrictedModelView(WindcavePaymentRequest, db.session, category='Admin')
)
...
admin.add_view(
    FiatDbTransactionModelView(FiatDbTransaction, db.session, category='Admin')
)
admin.add_view(
    WithdrawalConfirmationView(WithdrawalConfirmation, db.session, category='Admin')
)
...
if app.config['USE_REFERRALS']:
    admin.add_view(
        RestrictedModelView(Referral, db.session, category='Admin', name='Referrals')
    )
    admin.add_view(
        BaseOnlyUserOwnedModelView(
            Referral,
            db.session,
            category='User',
            name='Referrals',
            endpoint='UserReferrals',
        )
    )
