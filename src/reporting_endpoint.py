...
def user_counting(table, start_date, end_date):
    result = table.query.filter(
        and_(table.confirmed_at >= str(start_date), table.confirmed_at <= str(end_date))
    ).count()
    if not result:
        result = 0
    return result
...
def broker_order_amount(table, start_date, end_date, market):
    result = (
        table.query.filter(table.market == market)
        .filter(and_(table.date >= str(start_date), table.date < str(end_date)))
        .with_entities(func.sum(table.base_amount))
        .scalar()
    )
    if not result:
        result = 0
...
def broker_order_count_lifetime(table, market):
    result = table.query.filter(table.market == market).count()
    if not result:
        result = 0
    return result
...
def dashboard_general():
    ...
    if balances:
        for balance in balances:
            balance_formatted = dict(
                symbol=balance.symbol,
                available=assets.asset_dec_to_str(balance.symbol, balance.available),
                total=assets.asset_dec_to_str(balance.symbol, balance.total),
            )
            balances_formatted.append(balance_formatted)
    return render_template(
        'reporting/dashboard_general.html', dasset_balances=balances_formatted
    )
...
def dashboard_user():
    ...
    user_count_monthly = user_counting(
        User, FIRST_DAY_CURRENT_MONTH(), FIRST_DAY_NEXT_MONTH()
    )
    user_count_yearly = user_counting(
        User, FIRST_DAY_CURRENT_YEAR(), FIRST_DAY_NEXT_YEAR()
    )
    return render_template(
        'reporting/dashboard_user.html',
...
def dashboard_report_broker_order():
    ...
    for market in assets.MARKETS:
        ...
        order_count_yesterday = broker_order_count(
            BrokerOrder, YESTERDAY(), TODAY(), market
        )
        order_count_week = broker_order_count(
            BrokerOrder, MONDAY(), NEXT_MONDAY(), market
        )
        order_count_month = broker_order_count(
            BrokerOrder, FIRST_DAY_CURRENT_MONTH(), FIRST_DAY_NEXT_MONTH(), market
        )
        order_count_year = broker_order_count(
            BrokerOrder, FIRST_DAY_CURRENT_YEAR(), FIRST_DAY_NEXT_YEAR(), market
        )
        order_count_lifetime = broker_order_count_lifetime(BrokerOrder, market)
        order_amount_today = broker_order_amount(
            BrokerOrder, TODAY(), TOMORROW(), market
        )
        order_amount_yesterday = broker_order_amount(
            BrokerOrder, YESTERDAY(), TODAY(), market
        )
        order_amount_week = broker_order_amount(
            BrokerOrder, MONDAY(), NEXT_MONDAY(), market
        )
        order_amount_month = broker_order_amount(
            BrokerOrder, FIRST_DAY_CURRENT_MONTH(), FIRST_DAY_NEXT_MONTH(), market
        )
        order_amount_year = broker_order_amount(
            BrokerOrder, FIRST_DAY_CURRENT_YEAR(), FIRST_DAY_NEXT_YEAR(), market
        )
        order_amount_lifetime = broker_order_amount_lifetime(BrokerOrder, market)
        orders_data[market] = dict(
            ...
            first_day_current_year=first_day_current_year,
            first_day_next_year=first_day_next_year,
            order_count_today=order_count_today,
            order_count_yesterday=order_count_yesterday,
            ...
            order_amount_week=order_amount_week,
            order_amount_month=order_amount_month,
            order_amount_year=order_amount_year,
            order_amount_lifetime=order_amount_lifetime,
        )
    return render_template(
        'reporting/dashboard_broker_orders.html', orders_data=orders_data
    )
