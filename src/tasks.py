    ...
    for balance in data:
        if balance.symbol == 'NZD':
            if balance.available < app.config['MIN_AVAILABLE_NZD_BALANCE']:
                balance_format = assets.asset_dec_to_str(
                    balance.symbol, balance.available
                )
                subject = (
                    f'Available {balance.symbol} Balance below the minimum threshold'
                )
                ...
                email_utils.email_notification_alert(
                    logger, subject, msg, app.config['ADMIN_EMAIL']
                )
    ...
    while True:
        try:
            ...
            if err:
                ...
                gevent.sleep(
                    2, False
                )  # probably timeout so we wait a short time before polling again
            else:
                ...
                with app.app_context():
                    if pay and pay['status'] == 'paid':
                        ...
                        deposit = CryptoDeposit.from_wallet_reference(
                            db.session, bolt11
                        )
                        email = None
                        if deposit:
                        ...
                        websocket.ln_invoice_paid_event(
                            label, payment_hash, bolt11, email, description, sat
                        )
...
task_manager.repeated(
    'deposits, withdrawals, orders', process_depwith_and_broker_orders, 5
)
