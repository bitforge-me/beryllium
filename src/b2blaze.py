...
def backblaze_auth_headers():
    ...
    creds = base64.b64encode(
        (B2_ACCOUNT_ID + ':' + B2_APPLICATION_KEY).encode('ascii')
    ).decode('ascii')
...
def backblaze_get_upload_url(api_url, auth_token, bucket_id):
    ...
    r = requests.post(
        api_url + '/b2api/v2/b2_get_upload_url', headers=headers, json=body
    )
    r.raise_for_status()
    data = r.json()
...
def backblaze_upload_file(
    upload_url, upload_auth_token, filename, bytesio, content_type
):
...
def backup_file(bucket, filename, bytesio, content_type):
    try:
        ...
        upload_url, upload_auth_token = backblaze_get_upload_url(
            api_url, auth_token, bucket_id
        )
        backblaze_upload_file(
            upload_url, upload_auth_token, filename, bytesio, content_type
        )
        return True
